...
exports.disable_enable_User = async (req, res) => {
    try {
        let user = await userModel.findOne({ _id: req.params.id })
        if (user) {
            let isActive = user.active
            console.log(isActive);
            user.set({ active: isActive ? false : true });
            await user.save();
            return res.json({ message: isActive ? "successfully disabled " : "successfully enabled" })
        }
